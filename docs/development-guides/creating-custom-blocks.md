...
```bash
npm run scaffold:block -w=blank-brand-theme
```
 ...
 * Block markup
 *
 * @package BlankBrandTheme\Blocks\MyBlock
 *
...
/**
 * Title: My Pattern
 * Slug: blank-brand-theme/my-pattern
  ...
  return (
    <>
      <BlockControls group="block">
        <ToolbarButton
          icon={alignLeft}
          title={__('Align Left', 'blank-brand-theme')}
          onClick={() => setAttributes({ alignment: 'left' })}
        />
      </BlockControls>
      <InspectorControls group="settings">
        <PanelBody title={__('Block Settings', 'blank-brand-theme')}>
      ...
      <div {...blockProps}>
        <RichText
          ...
          value={title}
          onChange={(title) => setAttributes({ title })}
          placeholder={__('Enter title...', 'blank-brand-theme')}
        />
      </div>
    </>
  );
