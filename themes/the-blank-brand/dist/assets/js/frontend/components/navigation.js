...
function initNavigation() {
    ...
    if (!nav)
        return;
    const toggles = Array.from(nav.querySelectorAll(TOGGLE_SEL));
    if (!toggles.length)
        return;
    // ------------------------------------------------------------------
    // Helpers
    // ------------------------------------------------------------------
    function getParent(toggle) {
        return toggle.closest(PARENT_SEL);
    }
    ...
    document.addEventListener('click', (e) => {
        if (!e.target.closest(PARENT_SEL)) {
            closeAll();
        }
    });
    // ------------------------------------------------------------------
    ...
    nav.addEventListener('keydown', (e) => {
        if (!['ArrowDown', 'ArrowUp'].includes(e.key))
            return;
        const openParent = nav.querySelector(`${PARENT_SEL}.${OPEN_CLASS}`);
        if (!openParent)
            return;
        const items = Array.from(openParent.querySelectorAll('.sub-menu a'));
        if (!items.length)
            return;
...
function initMinicart() {
    const toggle = document.querySelector(CART_TOGGLE_SEL);
    const panel = document.querySelector(CART_PANEL_SEL);
    if (!toggle || !panel)
        return;
    function openCart() {
