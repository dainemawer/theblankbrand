...
class Blocks implements ModuleInterface {
	...
	public function register_theme_blocks() {
		...
		if ( file_exists( BLANK_BRAND_THEME_BLOCK_DIST_DIR ) ) {
			$block_json_files = glob( BLANK_BRAND_THEME_BLOCK_DIST_DIR . '*/block.json' );
	...
	public function blocks_editor_styles() {
		wp_enqueue_style(
			...
			BLANK_BRAND_THEME_TEMPLATE_URL . '/dist/css/editor-style-overrides.css',
			[],
		...
		if ( defined( 'SCRIPT_DEBUG' ) && SCRIPT_DEBUG ) {
			wp_enqueue_script(
				'editor-style-overrides',
				BLANK_BRAND_THEME_TEMPLATE_URL . '/dist/js/editor-style-overrides.js',
	...
	public function enqueue_block_specific_styles() {
		$stylesheets = glob( BLANK_BRAND_THEME_DIST_PATH . 'blocks/autoenqueue/**/*.css' );
		...
		foreach ( $stylesheets as $stylesheet_path ) {
			$block_type = str_replace( BLANK_BRAND_THEME_DIST_PATH . 'blocks/autoenqueue/', '', $stylesheet_path );
			$block_type = str_replace( '.css', '', $block_type );
			$asset_file = BLANK_BRAND_THEME_DIST_PATH . 'blocks/autoenqueue/' . $block_type . '.asset.php';
			...
			wp_register_style(
				"blank-brand-theme-{$block_namespace}-{$block_name}",
				BLANK_BRAND_THEME_DIST_URL . 'blocks/autoenqueue/' . $block_type . '.css',
			...
			wp_enqueue_block_style(
				$block_type,
				[
					'handle' => "blank-brand-theme-{$block_namespace}-{$block_name}",
	...
	public function register_block_pattern_categories() {
		register_block_pattern_category(
			'10up-theme',
			[ 'label' => __( '10up Theme', 'blank-brand-theme' ) ]
		);

		register_block_pattern_category(
			'blank-brand-theme',
			[
				'label'       => __( 'The Blank Brand', 'blank-brand-theme' ),
				'description' => __( 'Homepage and page section patterns.', 'blank-brand-theme' ),
			]
		);
	}
}
